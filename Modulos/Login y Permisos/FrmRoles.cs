...
namespace Reportes
{
	public partial class FrmRoles : Form
	{
		...
		private void prueba_Load(object sender, EventArgs e)
		{
			...
			DataTable usuarios = con.GetQuery("select userid, name from users order by name asc;");
		...
		{
			ClsLoginVerification cls = new ClsLoginVerification(ConfigurationManager.ConnectionStrings["log"].ConnectionString);
			if (MessageBox.Show("Estas seguro de eliminar este usuarios?", "Eliminar usuario", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
				== DialogResult.Yes)
			{
				cls.EliminarUsuario(comboBox1.SelectedValue.ToString());
				MessageBox.Show("Se ha eliminado el usuario exitosamente", "Exito!", MessageBoxButtons.OK, MessageBoxIcon.Information);
				prueba_Load(sender, e);
			}
		}
	}
